...
**Assumption LG (nonsingular linear-Gaussian) — in force throughout.** The
observational law `P` is a **nonsingular jointly Gaussian** distribution over
`V`, compatible with `Ĉ`, generated by a linear-Gaussian SCM whose DAG lies in
`[Ĉ]`:
...
Both clauses are load-bearing and neither is inherited from `r_val`.
*Nonsingularity* makes `β(S;Σ)` well defined: the normal equations have a unique
solution exactly when the relevant leading submatrix of `Σ` is invertible, and
every principal submatrix of a positive-definite matrix is positive definite.
*Linearity* is what makes that coefficient **equal to the causal effect** when
`S` is a valid adjustment set; in a general SCM a valid adjustment set
identifies the effect through the g-formula, which is not an OLS coefficient.
§6 states precisely what survives outside this class — less than earlier drafts
of this document claimed. For a set `S ⊆ V \ {X,Y}` write
...
the population level, for every observational law satisfying Assumption LG and
compatible with `Ĉ`.
...
in every `D ∈ [G]`. Fix such a `D`. **Under Assumption LG** a valid adjustment set
identifies the total effect through the population least-squares coefficient:
`β(Z; Σ) = τ_D`. This is the one step of the argument that consumes linearity,
and `Σ ≻ 0` is what makes the coefficient unique. Hence `τ_D = θ_Z` for every
...
|---|---|---|---|
| **LG** | nonsingular jointly Gaussian law from a linear-Gaussian SCM, `Σ ≻ 0` | **ASSUMED** | §1 — *required; not inherited from `r_val`* |
| A | `B ≡ 0` on the ball of radius `r_val − 1` | **PROVED** | definition of `r_val`; **LG** (validity ⟹ identification by OLS) |
| B | `G ⪯ H ⟹ B(G) ≤ B(H)` | **PROVED** | definition of the order only (**LG** for well-definedness of `B`) |
...
| E | semi-local `T(G)`, no DAG enumeration | **PROVED** | Meek's theorem; **LG** for the closed-form cost |
...
deliberate: **`r_ε` inherits the *order-theoretic* chain of `r_val` and adds
nothing to it.** Any future strengthening or weakening of that chain moves both
radii together. It does, however, add **Assumption LG** (§1): `r_val` is defined
by a graphical property and holds for any law compatible with `Ĉ`, whereas `r_ε`
is defined through OLS coefficients and needs the nonsingular linear-Gaussian
class. Earlier wording in this document claimed no assumption was added at all;
that was an error, and the corrected accounting is in the closing paragraph of
this section. In particular the one-sided error direction is unchanged — an upward
...
**Beyond linear-Gaussian — a correction.** An earlier version of this paragraph
claimed that Theorems A–E "go through verbatim" outside the linear-Gaussian
class, and that linearity was a statement about the *cost* of `B` rather than
its *validity*. **That claim was wrong**, and it is withdrawn. The distinction
it missed is between the order-theoretic skeleton and the estimand.

*What is genuinely free of linearity.* The order-theoretic content — Theorem B,
Corollaries B1/B2, Theorem U, Proposition R, Theorem C, Theorem D — uses only
that `B(G)` is a maximum over a set indexed by `[G]`, and that `[·]` grows as
knowledge is withdrawn. Any functional `τ_·` with that shape inherits them. This
is a statement about the *architecture*, and it is what licenses reusing the
retraction search unchanged.

*What is not.* Theorem A is an identification statement and it **does** use
linearity, in the definition of the objects and not merely in their evaluation.
`θ_Z` and `τ_D` are defined in §1 as population **OLS coefficients**. In a
general SCM a valid adjustment set identifies the total effect through the
g-formula

```
E[Y | do(X=x)]  =  Σ_s  E[Y | X=x, S=s] · P(S=s) ,
```

which coincides with the coefficient on `X` in a regression of `Y` on `{X} ∪ S`
only when the outcome regression is linear in `X` with no `X`–`S` interaction.
Drop linearity and `β(Z;Σ) = τ_D` is simply false in general, so Theorem A — and
with it the zero-bias guarantee that is the whole point of the certificate —
fails for the estimand as defined. Nonsingularity is needed even to define
`β(S;Σ)` uniquely.

*The honest generalisation, and its price.* One may redefine `τ_D` and `θ_Z` as
g-formula contrasts adjusting for `pa_D(X)` and `Z` respectively. Under any SCM
with positivity, Definitions 1–2 are then well posed, Theorem A recovers its
proof, and B, B1, B2, U, R, C, D, E follow. But this is a **different
estimand**, not the same one in greater generality: the certificate then covers
a g-formula estimate and says nothing about a reported regression coefficient,
and the closed-form evaluation of Theorem E — `2^{|nb(X)|}` linear solves, no
DAG enumeration — is lost, because each `τ_D` becomes a nonparametric
regression. The cost claims of §5 are therefore **restricted to the
linear-Gaussian regime** and are not to be quoted outside it. The empirical
study of §7 is linear-Gaussian for the same reason.
