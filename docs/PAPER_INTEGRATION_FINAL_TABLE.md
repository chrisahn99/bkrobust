...
3. **Never write `r_val` in paper-facing prose.** `PAPER_NARRATIVE.md` §11 mandates `r_hop`.
   The code, `epsilon-radius.tex` and `PAPER_INTEGRATION_R_EPSILON.md` all still say `r_val`;
   that is an existing inconsistency in the repo, not a licence to propagate it. This brief
   writes `r_hop` in every paper-facing sentence and `r_val` only when naming a code symbol
   or a JSON key.
...
**`r_hop = 1` almost everywhere.** Of the informative instances, 46/53 have `r_hop = 1`, and
...
`r_hop` at every ε in `{0.01, 0.05, 0.10, 0.25, 0.50, 1.00}` for all but 2 of 53 informative
...
Among the 53 informative instances with a finite radius, the distribution of `r_hop` is:

| `r_hop` | 1 | 2 | 3 | 4 | 11 |
|---|---|---|---|---|---|
| instances | 46 | 2 | 3 | 1 | 1 |

**46/53 = 87% sit at `r_hop = 1`.** Taking networks as the unit of analysis, as
...
`r_eps` exceeds `r_hop` at some ε in only **2 of 53** informative instances (`Kampen_2014`,
`Schipf_2010`). At network level the median `r_eps` equals the median `r_hop` at every ε in
...
- **This table is not pre-registered.** `docs/EVALUATION_PLAN.md` names `r_claim`, `r_hop`,
...
- **`r_eps` on real networks is SEM-conditional; `r_hop` is not.** The corpus ships graph
  structure only, so Σ comes from a seeded linear-Gaussian SEM attached to the ground-truth
  DAG (`demo.evaluate.random_sem`). `r_hop` is purely graph-theoretic and carries no such
...
- **The ε-null result is about this corpus and this knowledge source.** It does not show the
  ...
  separate from `r_hop` here", not "does not separate".
...
**6.4 The `r_val` → `r_hop` rename.** `PAPER_NARRATIVE.md` §11 forbids `r_val` in the paper,
while `epsilon-radius.tex` and `PAPER_INTEGRATION_R_EPSILON.md` still use it throughout. This
brief follows the mandate, which makes it locally inconsistent with the two documents it sits
beside. Resolve the rename repo-wide, or accept the inconsistency knowingly.
