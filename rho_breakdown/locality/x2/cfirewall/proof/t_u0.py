"""Is U vacuous?  And is the stronger U0 true?"""
import sys
            ...
            for (a, b) in ((a0, b0), (b0, a0)):
                ...
                for (x, y, (cn0, fb0, pa0, O0, _)) in Q:
                    ...
                    if S:
                        ...
                        if not (S <= fb1): acc["U_forb_viol"] += 1
                        Snc = S - cn1
                        if Snc and len(wit["S"]) < 8:
                            wit["S"].append((G0.tolist(), H.tolist(), a, b, x, y,
                                             sorted(Snc), sorted(cn1), sorted(fb1)))
                    # how far can it go: is pa1 cap pd0 nonempty & moved?
                    if S and O1 != O0: acc["nonvacuous_moved"] += 1
...
def main():
    ...
    json.dump({"counts": dict(tot), "wit": {k: v for k, v in W.items()}},
              open("t_u0_p%d.json" % p, "w"), indent=1)
