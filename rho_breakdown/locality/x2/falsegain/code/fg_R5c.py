...
def job(arg):
    ...
    ref = v_structures(C)
    mp = reachable_mpdags(C, 400)
    if mp is None: return {("SKIPPED",):1}, []
...
def main():
    ...
    json.dump(dict(p=p,n_cpdags=len(keys),counts=out,witnesses=wits),
              open(f"/Users/josecosta/mugango/output/2026-08-19_x1-x2-locality-spurious/x2/falsegain/code/fgR_p{p}_cap400.json","w"),indent=1)
