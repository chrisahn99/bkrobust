...
NETS = ("ecoli70", "arth150", "magic-niab", "magic-irri")
    ...
    for cov in (1.0, 0.5):
        ...
        for name in NETS:
            ...
            print(f"{name:>12} {len(rows):5d} {100*len(fin)/len(rows):7.1f}% {st:6.1f}% | "
                  ...
                  + f"  [{time.time()-t:.0f}s]")
    json.dump(allrows, open("sweep.json", "w"), indent=1)
    print("\nColunas pt/bd = % das replicas em que o raio calculado de Sigma-hat "
          "COINCIDE com o raio do oraculo.")
    print(f"{len(allrows)} instancias, {REPS} replicas por tamanho -> sweep.json")
